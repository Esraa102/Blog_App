...
export const fetchData = async (query = "") => {
  ...
  const filteredPosts = posts.filter((post) => {
    return post.title.toLowerCase().includes(query.toLocaleLowerCase());
  });
...
export const editPost = async (editedPost) => {
  await new Promise((resolve) => setTimeout(resolve, 1000));
  const post = posts.filter((post) => post.id === editedPost.id);
  post[0].title = editedPost.post.title;
  post[0].body = editedPost.post.body;
  return post[0];
};
